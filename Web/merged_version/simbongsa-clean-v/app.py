...
def already_liked(user_id, content_id):
    ...
    for i in range(len(a)):
        if a[i].user_id == int(user_id) and a[i].content_id == int(content_id):   ##### type 맞춰주기!!!!!
            res = [a[i].user_id, a[i].content_id]


#    print(type(content_id), type(a[0].content_id))
    if len(res) == 0:  # db에 없으면 false
        print(user_id, content_id, 'false')
        return 'false'
    else:              # db에 있으면 true
        return 'true'
...
#    print('city_first',first_city_list)
    first_city4 = first_city_list[:4]
...
#    print(len(first_city4))
    print('city_list',first_city_list)
...
def more_page(name=None):
    ...
    if name == 'recommend':
        ...
        recom_categories = []
        for i in range(len(more_recommend)):
            recom_categories.append(max_classification(vol_list[more_recommend[i][0]]))


        return render_template('more_recommend.html', vol_content=vol_content, category_name=recom_categories, check_flag=likes_bool,star_rate=similar_star_rate_,
        ...
        print('vol_c',vol_content[major_similarity[0][0]][1])
        ...
        return render_template('more_major.html', vol_content=vol_content,category_name=major_categories, check_flag=likes_bool,star_rate=similar_star_rate_,
                               ...
                               major_list=major_similarity)
                               #city=first_city_list)
